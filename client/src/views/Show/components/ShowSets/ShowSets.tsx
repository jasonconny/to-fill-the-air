...
import styles from './ShowSets.scss';
...
export const ShowSets: React.FC<SetProps> = ({ sets }) => (
  <>
    {sets.length === 1 ? (
      <ul className={classNames(styles.list, styles.listSingle)}>
        {sets[0].songs.map((song, index) => (
          <li className={styles.listItem} key={`${sets[0].set_id}-${index}`}>
            {song.title}
      ...
      <dl className={styles.setsContainer}>
        {sets.map((set) => (
          <React.Fragment key={set.set_id}>
            <dt className={styles.setLabel}>{set.name}</dt>
